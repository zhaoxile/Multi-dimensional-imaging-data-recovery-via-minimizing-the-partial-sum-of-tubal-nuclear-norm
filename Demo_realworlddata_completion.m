...
for i = 1:size(X,3)
SSIMv1(i) = ssim(Xhat1(:,:,i),X(:,:,i));
PSNRv1(i) = psnr(Xhat1(:,:,i),X(:,:,i));
end
mSSIM1 = mean(SSIMv1);mPSNR1 = mean(PSNRv1);
...
for i = 1:size(X,3)
SSIMv2(i) = ssim(Xhat2(:,:,i),X(:,:,i));
PSNRv2(i) = psnr(Xhat2(:,:,i),X(:,:,i));
end
mSSIM2 = mean(SSIMv2);mPSNR2 = mean(PSNRv2);

%% Tensor completion based on PSTNN
rho = 0.01;% tune this parameter to controal the estimated rank N
[rankN,~] = prox_rankN(X,rho);%n*ones(1,n3);%
[Xhat3,~,~,~] =  LRTC_PSTNN(M,omega,opts,rankN);%,Xhat2);%
...
disp(['PSNR || ' num2str(PSNR0) ' ||  ' num2str(mPSNR1) ' ||  ' num2str(mPSNR2) ' ||  ' num2str(mPSNR3) ]);
disp(['SSIM || ' num2str(SSIM0) ' ||  ' num2str(mSSIM1) ' ||  ' num2str(mSSIM2) ' ||  ' num2str(mSSIM3)  ]);
